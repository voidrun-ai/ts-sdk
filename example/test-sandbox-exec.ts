  ...
  try {
    ...
    await step('Streaming exec (SSE)', async () => {
      ...
      await sandbox.exec(
        {
          command:
            'seq 1 10 | while read i; do echo "streaming output $i"; sleep 1; done',
        },
        {
          onStdout: (data) => {
    ...
    await step('Create file in /tmp', () =>
      sandbox.exec({
        command:
          "echo 'test content' > /tmp/exec-test.txt && cat /tmp/exec-test.txt",
      }),
    );
    ...
    await step('Command with non-zero exit code', async () => {
      ...
      if (result.data?.exitCode !== 0) {
        console.log(
          `Got expected non-zero exit code: ${result.data?.exitCode}`,
        );
      }
      return result;
    });
    ...
    await step('Check disk space', () => sandbox.exec({ command: 'df -h' }));
