...
async function runTests() {
  ...
  try {
    ...
    console.log(
      `✅ Found process in list: PID ${ourProcess.pid}, command: ${ourProcess.command}\n`,
    );
    ...
    if (output.length > 0) {
      ...
      console.log(
        `⚠️ No stdout captured - process may have completed before attach`,
      );
    }
    console.log();
    ...
    if (!waitResult.success || waitResult.exitCode !== 0) {
      console.log(
        `⚠️ Unexpected exit code: ${waitResult.exitCode} (may indicate process already exited)`,
      );
    } else {
