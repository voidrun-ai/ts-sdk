...
async function main() {
  ...
  try {
    ...
    let result = await sdbx.runCode('console.log(5 * 7)', {
      language: 'javascript',
    });
    ...
    result = await sdbx.runCode('const i = 444; console.log("Looping", i)', {
      language: 'javascript',
    });
    ...
    const lastNumber = factorialMatch
      ? factorialMatch[factorialMatch.length - 1]
      : null;
    if (lastNumber !== '720') {
      throw new Error(
        `Unexpected output for factorial(6): got ${result.stdout}`,
      );
    }
  } catch (error) {
