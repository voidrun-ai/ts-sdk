...
async function testEmptyProcessList() {
  ...
  try {
    ...
    if (response.processes === null) {
      console.error(
        '❌ FAIL: response.processes is null (should be empty array [])',
      );
    } else if (
      ...
      console.error(
        '❌ FAIL: Unexpected response.processes value:',
        response.processes,
      );
    }
    ...
    console.log(
      'Raw processes response:',
      JSON.stringify(responseAfter, null, 2),
    );
    ...
    if (responseAfter.processes === null) {
      ...
      Array.isArray(responseAfter.processes) &&
      responseAfter.processes.length === 0
    ) {
      console.log(
        '✅ PASS: response.processes is still empty array [] (finished processes pruned)',
      );
    } else {
      console.error(
        '❌ FAIL: Unexpected response.processes value:',
        responseAfter.processes,
      );
    }
