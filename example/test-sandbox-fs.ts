...
async function main() {
  ...
  try {
    ...
    await step('changePermissions', () =>
      fs.changePermissions(filePath, '644'),
    );
    ...
    const archiveResp = await step('compressFile', () =>
      fs.compressFile(baseDir, 'tar.gz'),
    );
    ...
    await step('extractArchive', () =>
      fs.extractArchive(archivePath, extractDir),
    );
    ...
    const downloaded = await step('downloadFile', () =>
    ...
    const downloadBuffer = Buffer.isBuffer(downloaded)
      ? downloaded
    ...
    console.log(
      `Saved downloaded file to ${downloadPath} (${savedSize} bytes)`,
    );
    console.log(`Downloaded content preview:\n${savedContent}`);

    await step('deleteFile moved', () => fs.deleteFile(movedPath));
    await step('deleteFile stream.txt', () =>
      fs.deleteFile(`${baseDir}/stream.txt`),
    );
