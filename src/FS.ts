...
function detectRuntime(): 'node' | 'deno' | 'bun' | 'unknown' {
  ...
  // Deno
  if (
    typeof globalThisAny.Deno !== 'undefined' &&
    globalThisAny.Deno?.version
  ) {
    return 'deno';
  }
...
export class FS {
  ...
  async compressFile(
    path: string,
    format: 'tar' | 'tar.gz' | 'tar.bz2' | 'zip' = 'tar.gz',
  ) {
    const response = await wrapRequest(
      this.fsApi.compressFile({
        id: this.sandboxId,
        path,
        format,
      }),
  ...
  async folderSize(path: string) {
    ...
    const info: FolderSizeResponse = {} as FolderSizeResponse;
    if (result?.data?.stdout) {
